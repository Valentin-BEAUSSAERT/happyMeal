...
function fillModalWithRecipe(index) {
  ...
  function handleLikeButtonClick() {
    ...
    const recetteJSON = JSON.stringify({
      titre: modalTitle.textContent,
      contenu: modalContent.innerHTML,
    });

    // Changer la couleur du bouton et stocker la modal dans le stockage local
    if (likeButton.style.color === "red") {
      likeButton.style.color = "black";
      localStorage.removeItem("favoris");
    } else {
      likeButton.style.color = "red";
      localStorage.setItem("favoris", recetteJSON);
    }
  }

  // Mettez le titre de la recette avec le bouton like à côté
...
`;
  ...
  // Mettez les autres détails de la recette
