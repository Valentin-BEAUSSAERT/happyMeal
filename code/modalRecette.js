...
const data = {
    "recettes":[
        ...
        },
        {
           ...
           "temps_preparation":"20 minutes",
           "ingredients":[
              "Fruits variés (fraises, kiwis, oranges, raisins, etc.), coupés en morceaux",
        ...
        },
        {
           ...
           "etapes":[
              "Nettoyez et coupez les champignons en tranches.",
              "Dans une grande poêle, faites chauffer un peu d'huile d'olive et faites revenir l'oignon haché et l'ail émincé jusqu'à ce qu'ils soient translucides.",
              "Ajoutez les champignons et faites-les sauter jusqu'à ce qu'ils soient dorés.",
              "Ajoutez le riz Arborio et mélangez bien pour enrober chaque grain d'huile.",
              "Versez le vin blanc et laissez-le s'évaporer.",
              "Ajoutez une louche de bouillon de légumes chaud et remuez jusqu'à ce que le liquide soit absorbé.",
              "Continuez à ajouter le bouillon louche par louche, en remuant fréquemment, jusqu'à ce que le riz soit crémeux et cuit al dente, cela prendra environ 18-20 minutes.",
              "Incorporez le beurre et le Parmesan râpé.",
              "Assaisonnez avec du sel et du poivre selon votre goût.",
  ...
  // Supposons que cette fonction soit appelée avec l'index de la recette que l'utilisateur veut voir.
  function fillModalWithRecipe(index) {
    const recette = data.recettes[index];
    
    // Construisez le HTML pour les ingrédients
    let ingredientsHtml = '<ul>';
    for (const ingredient of recette.ingredients) {
      ingredientsHtml += `<li>${ingredient.quantite} de ${ingredient.nom}</li>`;
    }
    ingredientsHtml += '</ul>';
    
    // Construisez le HTML pour les étapes
    let etapesHtml = '<ol>';
    ...
    // Remplissez la modal avec les données de la recette
  ...
  // Ouvre la modal avec les détails de la recette pour le Poulet rôti aux herbes (première recette dans votre JSON)
  fillModalWithRecipe(0);
